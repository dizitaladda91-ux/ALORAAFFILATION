...
export const AdminWithdrawals = () => {
  ...
  const approvePayoutRequest = async (item) => { try { await approvePayout(item.id); showSuccess('Payout approved. A different administrator can now process it.'); load(); } catch (error) { showError(error.message || 'Payout approval failed.'); } };
  ...
  return <div className="financial-admin-page">
    ...
    <Card><div className="financial-card-heading"><div><h2>Payouts</h2><p>Record the bank reference before processing or completing a payout.</p></div><Filter size={18} /></div>
      ...
      <div className="admin-withdrawal-list">{payouts.items?.map((item) => <article className="bank-account-item financial-record" key={item.id}><div><strong>{item.payout_number} · {formatCurrency(item.amount)}</strong><p>{item.gateway} · {item.transaction_reference || 'Reference not recorded'}</p><small>Created {formatDate(item.created_at)}</small><small>Approval: {item.approval_status || 'PENDING'}</small>{item.failure_reason && <small className="financial-error">Failure reason: {item.failure_reason}</small>}</div><div className="bank-account-actions"><Badge status={item.status}>{item.status}</Badge>{item.status === 'PENDING' && item.approval_status !== 'APPROVED' && <Button onClick={() => approvePayoutRequest(item)}>Approve</Button>}{item.status === 'PENDING' && item.approval_status === 'APPROVED' && <Button onClick={() => openDialog('process', item)}>Process</Button>}{item.status === 'PROCESSING' && <Button onClick={() => openDialog('complete', item)}>Complete</Button>}{['PENDING', 'PROCESSING'].includes(item.status) && <Button variant="danger" onClick={() => openDialog('fail', item)}>Fail</Button>}</div></article>)}{!loading && payouts.items?.length === 0 && <p className="empty-state">No payouts match this filter.</p>}</div>
