...
export const openRazorpayCheckout = async ({ order, customer, onSuccess, onFailure }) => {
  ...
  const checkout = new window.Razorpay({ key: order.keyId, order_id: order.orderId, amount: order.amount, currency: order.currency, name: 'Alora Radiance', prefill: customer, handler: onSuccess, modal: { ondismiss: onFailure } });
  checkout.open();
};
